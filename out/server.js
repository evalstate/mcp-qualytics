...
async function findTypeScriptFiles(dir, additionalIgnorePatterns = []) {
    const files = [];
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    switch (request.params.name) {
        case "typescript_analyze_text": {
            const { code, format = 'text' } = request.params.arguments;
            try {
                ...
                if (format === 'table') {
                    ...
                    for (const fn of analysis.functions) {
                        tableRows.push(`| function | ${fn.name} | ${fn.type} | ${fn.startLine}-${fn.endLine} | ${fn.metrics.linesOfCode} | ${fn.metrics.cyclomaticComplexity} | ${fn.metrics.maintainabilityIndex.toFixed(2)} | - | ${fn.metrics.methodCount} | ${fn.metrics.averageMethodComplexity.toFixed(2)} | - |`);
                    }
                    return {
...
Functions:`;
  ...
  - Average Method Complexity: ${fn.metrics.averageMethodComplexity.toFixed(2)}`).join('\n');
                    return {
        ...
        case "typescript_analyze_file": {
            const { filepath, format = 'text' } = request.params.arguments;
            try {
...
Functions:`;
  ...
  - Average Method Complexity: ${fn.metrics.averageMethodComplexity.toFixed(2)}`).join('\n');
        ...
        case "typescript_analyze_directory": {
            const { directory, include_functions = true, ignore_patterns = [] } = request.params.arguments;
            try {
                ...
                for (const file of files) {
                    ...
                    tableRows.push(`| ${relativePath} | file | - | - | - | ${analysis.fileMetrics.linesOfCode} | ${analysis.fileMetrics.cyclomaticComplexity} | ${analysis.fileMetrics.maintainabilityIndex.toFixed(2)} | ${analysis.fileMetrics.classCount} | ${analysis.fileMetrics.methodCount} | ${analysis.fileMetrics.averageMethodComplexity.toFixed(2)} | ${analysis.fileMetrics.depthOfInheritance} |`);
                    // Add function-level metrics if requested
                    if (include_functions) {
                        for (const fn of analysis.functions) {
                            tableRows.push(`| ${relativePath} | function | ${fn.name} | ${fn.type} | ${fn.startLine}-${fn.endLine} | ${fn.metrics.linesOfCode} | ${fn.metrics.cyclomaticComplexity} | ${fn.metrics.maintainabilityIndex.toFixed(2)} | - | ${fn.metrics.methodCount} | ${fn.metrics.averageMethodComplexity.toFixed(2)} | - |`);
                        }
                    }
                }
                return {
